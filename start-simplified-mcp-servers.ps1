# Start Simplified MCP Servers
# This script starts all the simplified MCP servers

Write-Host "Starting simplified MCP servers..." -ForegroundColor Green
...
# Kill any existing Node.js processes on the MCP server ports
Write-Host "Killing any existing Node.js processes on MCP server ports..." -ForegroundColor Yellow

$ports = @(3011, 3020, 3021, 8020)
foreach ($port in $ports) {
    $process = Get-NetTCPConnection -LocalPort $port -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess
    if ($process) {
        $processName = (Get-Process -Id $process -ErrorAction SilentlyContinue).Name
        if ($processName -eq "node") {
            Write-Host "Killing Node.js process on port $port (PID: $process)" -ForegroundColor Yellow
...
$ollamaStarted = Start-MCPServer -name "ollama-mcp" -command "node simple-server.js" -workingDir "D:\mcp\nexus\mcp-servers\ollama-mcp" -checkUrl "http://localhost:3011/mcp/status"

# Start Code Enhancement MCP Server
$codeEnhancementStarted = Start-MCPServer -name "code-enhancement-mcp" -command "node simple-server.js" -workingDir "D:\mcp\nexus\mcp-servers\code-enhancement-mcp" -checkUrl "http://localhost:3020/mcp/status"

# Start Lucidity MCP Server
$lucidityStarted = Start-MCPServer -name "lucidity-mcp" -command "node dist/index.js" -workingDir "D:\mcp\nexus\mcp-servers\lucidity-mcp" -checkUrl "http://localhost:3021/mcp/status"

# Start Benchmark MCP Server
$benchmarkStarted = Start-MCPServer -name "benchmark-mcp" -command "node dist/index.js" -workingDir "D:\mcp\nexus\mcp-servers\benchmark-mcp" -checkUrl "http://localhost:8020/mcp/status"
