...
export class TemplateManager {
  ...
  public async initialize(): Promise<void> {
    ...
    try {
      ...
      logger.error(`Failed to initialize template manager: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
   ...
   */
  public async getTemplate(type: string): Promise<AgentTemplate | null> {
    ...
    if (fs.existsSync(templatePath)) {
      try {
        ...
        logger.error(`Failed to load template: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
    
    // Try to find a similar template
    const similarType = Array.from(this.templates.keys()).find(t => 
      t.toLowerCase().includes(type.toLowerCase()) || 
      type.toLowerCase().includes(t.toLowerCase())
    );
    
    if (similarType) {
      logger.info(`Using similar template ${similarType} for type ${type}`);
      return this.templates.get(similarType)!;
    }
    
    // Use generic template as fallback
    if (this.templates.has('generic')) {
      logger.info(`Using generic template for type ${type}`);
   ...
   */
  public async createTemplate(template: AgentTemplate): Promise<void> {
      ...
      for (const file of files) {
        if (file.endsWith('.json')) {
          ...
          try {
            ...
            logger.warn(`Failed to parse template file: ${filePath}`);
          }
        }
      }
      
      logger.info(`Loaded ${this.templates.size} templates from storage`);
    } catch (error) {
  ...
  private async createDefaultTemplates(): Promise<void> {
    ...
    try {
      ...
      const genericTemplate: AgentTemplate = {
        name: 'generic',
  ...
  private async startWorking(): Promise<void> {
    ...
    this.agentCommunication.updateAgentStatus(this.agentId, 'idle');
      ...
      for (const template of specializedTemplates) {
        ...
        const specializedTemplate: AgentTemplate = {
          ...genericTemplate,
          name: template.name,
      ...
      logger.error(`Failed to create default templates: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
}
