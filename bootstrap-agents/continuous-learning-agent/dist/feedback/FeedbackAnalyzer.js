...
class FeedbackAnalyzer {
    ...
    async analyzeFeedback(feedbacks) {
        bootstrap_core_1.logger.info(`Analyzing ${feedbacks.length} feedback items`);
        try {
            ...
            for (const [agentId, agentFeedbacks] of feedbackByAgent.entries()) {
                ...
                const recommendations = this.generateRecommendations(agentFeedbacks, topIssues, topStrengths, sentimentAnalysis);
                // Create analysis
                const analysis = {
                    agentId,
            ...
            bootstrap_core_1.logger.error(`Failed to analyze feedback: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    }
    /**
     ...
     */
    generateRecommendations(feedbacks, topIssues, topStrengths, sentimentAnalysis) {
