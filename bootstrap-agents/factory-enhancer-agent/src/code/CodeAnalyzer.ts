...
export class CodeAnalyzer {
  ...
  public async analyzeCode(request: CodeAnalysisRequest): Promise<CodeAnalysisResult> {
    logger.info(`Analyzing ${request.language} code${request.filePath ? ` in ${request.filePath}` : ''}`);
    ...
    try {
      ...
      if (lucidityAdapter) {
        try {
          const lucidityResult = await lucidityAdapter.analyzeCode(
            request.code,
            request.language
          );
          ...
          logger.warn(`Failed to analyze code with Lucidity: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
      ...
      if (ollamaAdapter && (issues.length === 0 || request.analysisTypes.includes('maintainability'))) {
        try {
          ...
          logger.warn(`Failed to analyze code with Ollama: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
  ...
  public async analyzeFile(
    filePath: string,
    analysisTypes: ('quality' | 'security' | 'performance' | 'maintainability')[] = ['quality', 'security', 'performance', 'maintainability']
    ...
    try {
      ...
      logger.error(`Failed to analyze file ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
   ...
   */
  private async analyzeWithOllama(request: CodeAnalysisRequest): Promise<CodeIssue[]> {
    const ollamaAdapter = this.adapterManager.getFirstOllamaMCPAdapter();
    if (!ollamaAdapter) {
      return [];
    }
...
`;
    ...
    try {
      ...
      logger.warn(`Failed to parse Ollama analysis response: ${error instanceof Error ? error.message : String(error)}`);
    }
   ...
   * Convert Lucidity issues to our format
   */
  private convertLucidityIssues(lucidityIssues: any[]): CodeIssue[] {
    return lucidityIssues.map(issue => {
      ...
      let category: 'quality' | 'security' | 'performance' | 'maintainability' = 'quality';
